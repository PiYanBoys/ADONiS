...
"""
def read_time_series(time_series, whichOne):
    main_set = pd.read_csv("/home/psxdp/workspace/TimeSeriesDatasets/"+str(time_series)+"/"+str(whichOne), header=None)
    return main_set[0]
"""
...
def main(time_series, my_experiment, repeats=1, antecedent_number=7, predict_past_points=9, est_past_points=9):
    ...
    if(time_series not in ["MG", "Lorenz"]):
         raise ValueError('The time series name can only be either MG or Lorenz and it must be specified in the run file.')
    ...
    noise_free_train = pd.read_csv("data/" + time_series + "/noise_free_train.csv", header=None)[0]
    
    #Read the noise free test set
    noise_free_test = pd.read_csv("data/"+str(time_series) + "/noise_free_test.csv", header=None)[0]
    ...
    for repeat in range(repeats):
        ...
        for index_test_set, test_set_name in enumerate(test_sets):
            ...
            if(my_experiment==1.2 or  my_experiment==2.2):
                ...
                noisy_training_set=get_noisy_set(test_set_name,noise_free_train[:-1],pd.Series.std(noise_free_train))
            ...
            temp_noisy_set=get_noisy_set(test_set_name,noise_free_test,pd.Series.std(noise_free_train))
            ...
            for index_advaced_NSFLSs, advaced_NSFLSs in enumerate(NSFLSs):
                ...
                for index_technique, my_technique in enumerate(techniques):
                    ...
                    test_obj.generate_firing_strengths(advaced_NSFLSs,my_technique,time_series,est_past_points)
